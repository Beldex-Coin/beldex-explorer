...
def lmq_connection():
    ...
    if lmq is None:
        ...
        lmq.start()
    if beldexd is None:
        beldexd = lmq.connect_remote(config.beldexd_rpc)
    return (lmq, beldexd)
...
class FutureJSON():
    ...
    lmq - the lmq object
    beldexd - the beldexd lmq connection id object
    ...
    def __init__(self, lmq, beldexd, endpoint, cache_seconds=3, *, cache_key='', args=None, fail_okay=False, timeout=10):
        ...
        if self.cache_key in cached and cached_args[self.cache_key] == args and cache_expiry[self.cache_key] >= datetime.now():
            ...
            self.json = None
            self.args = args
            self.future = lmq.request_future(beldexd, self.endpoint, [] if self.args is None else [self.args], timeout=timeout)
